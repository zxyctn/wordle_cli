...
```bash
integration_checker_model=$(node "./.gemini/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-integration-checker --raw)
```
...
```bash
NYQUIST_CONFIG=$(node "./.gemini/get-shit-done/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
